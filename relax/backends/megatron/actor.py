...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def _init(
        ...
        self._preserve_pg_for_weight_sync_eligible = self._all_ranks_agree(
            self._local_pg_preservation_eligibility()
        )
        if is_megatron_main_rank():
            logger.info(
    ...
    def _local_pg_preservation_eligibility(self) -> bool:
        """Conservative capability gate for the colocated IPC weight-sync path."""
        return bool(
            self.role == "actor"
            and self.args.colocate
            and self.args.offload_train
            and self._per_step_rollout
            and self._torch_memory_saver_enabled
    ...
    def _run_preserved_pg_canary(self) -> bool:
        """Verify the TP/DP communicators still work after TMS pause."""
        local_success = True
        try:
            groups = (
    ...
    def sleep(self, *, preserve_process_groups_for_weight_sync: bool = False) -> None:
        ...
        if preserve_process_groups:
            ...
            if not self._preserved_pg_canary_passed:
                ...
                if not self._preserved_pg_canary_passed:
                    logger.warning(
                        "[weight-sync-pg] disabling short-window preservation after canary failure"
                    )
                    self._preserve_pg_for_weight_sync_ready = False
                    destroy_process_groups()
